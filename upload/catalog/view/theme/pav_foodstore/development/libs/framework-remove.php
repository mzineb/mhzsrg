		...
		public function calculateSpans( $ospans=array(), $cols ){
			...
			for( $i=1; $i<= $cols; $i++ ){
				if( array_key_exists($i,$ospans) ){
					$spans[$i] = 'col-lg-'.$ospans[$i];
					
				}else{		
					if( (12-$tmp)%($cols-count($ospans)) == 0 ){
						$spans[$i] = "col-lg-".((12-$tmp)/($cols-count($ospans)));
						
					}else {
						if( $t == 0 ) {
							$spans[$i] = "col-lg-".( floor((11-$tmp)/($cols-count($ospans))) + 1 ) ;
						}else {
							$spans[$i] = "col-lg-".( floor((11-$tmp)/($cols-count($ospans))) + 0 ) ;
						}
						$t++;
					}					
				}
